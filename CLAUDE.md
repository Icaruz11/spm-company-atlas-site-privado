...
- **`tracking.js`** — thin layer over the always-on pixel. Exposes `window.SPMTracker` (`track`, `setConsent`, `getConsent`). `track()` fires `fbq` events whenever the pixel exists (used for `Lead` on form success). It never disables the pixel. The cookie consent banner was removed from the markup (tracking is always-on); the banner-wiring in `tracking.js` is now a defensive no-op.
